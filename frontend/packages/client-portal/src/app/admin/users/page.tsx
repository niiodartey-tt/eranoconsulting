...
export default function AdminUsersPage() {
  ...
  async function fetchUsers() {
    try {
      ...
      const res = await fetch(`${API_URL}/api/v1/admin/users/staff`, {
        headers: {
  ...
  async function handleCreateUser(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessage(`❌ ${err.detail || 'Failed to create user'}`);
        return;
      }
  ...
  async function handleDeleteUser(userId: number) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        setMessage(`❌ ${err.detail || 'Failed to delete user'}`);
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-900">Staff & Admin Accounts</h2>
          <button
